...
/**
 * Uses off-heap memory (i.e., memory not managed by GC) to store binary keys
 * and values.
 * 
 ...
 * 
 * By default the hash of the key using Arrays::hashCode is used to determin the
 * location of the entry. But alternative hash code functions can be used during
 * map construction.
 * 
 * The partition address points to the first address location of the first
 * partition. This address will be zero if the partition has yet to be
 * allocated. The next partition location can be found by moving the address 8
 * bytes down (assuming 64-bit addressing, otherwise 4 bytes).
 * 
 * At the partition location, an int (4 bytes) will first announce the size of
 * the partition. Thereafter two address values are used to indicate the address
 * of the entry key and the entry value.
 * 
 * At the entry key and entry value locations, an int is again used to announce
 * the size (in bytes) of either the key or value.
 * 
...
public class BOHMap implements Map<Binary, Binary> {
	 ...
	 * 
	 * The number of partitions should be scaled with the number of expected map
	 * entries, in order to maintain efficient lookup performance.
	 * 
	 ...
	 *            A positive number of partitions
	 * @param hashFunction
	 *            Hash function to use when calculating key hashes
	 */
	...
	public Binary get(Object key) {
		...
		for (long locationOffset = 0; locationOffset < entryCount; locationOffset++) {
			...
			// Check if we found the key
			if (isEqual) {
				long valueAddress = unsafe
						.getAddress(locationAddress + (locationOffset * addressSize * 2) + addressSize);
	...
	public Binary put(Binary key, Binary value) {
		...
		if (locationAddress == 0) {
			...
			locationAddress = unsafe.reallocateMemory(locationAddress,
					Integer.BYTES + (addressSize * 2 * (entryCount + 1)));
		}
	...
	private class ValuesIterator implements Iterator<Binary> {
		...
		public Binary next() {
			...
			long valueAddress = map.unsafe
					.getAddress(locationAddress + (locationOffset * map.addressSize * 2) + map.addressSize);
