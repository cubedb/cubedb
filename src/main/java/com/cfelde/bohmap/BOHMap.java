...
/**
 * Uses off-heap memory (i.e., memory not managed by GC) to store binary keys and values.
 *
 * <p>The internal memory structure is as follows:
 *
 * <p>A set of partitions (like in a hash map) are used where map entries are mapped to one of
 * these. The number of partitions are specified at map construction, and should be scaled according
 * to the expected number of entries stored in the map. Too few partitions and performance will
 * suffer, too many and you're wasting memory.
 *
 * <p>By default the hash of the key using Arrays::hashCode is used to determin the location of the
 * entry. But alternative hash code functions can be used during map construction.
 *
 * <p>The partition address points to the first address location of the first partition. This
 * address will be zero if the partition has yet to be allocated. The next partition location can be
 * found by moving the address 8 bytes down (assuming 64-bit addressing, otherwise 4 bytes).
 *
 * <p>At the partition location, an int (4 bytes) will first announce the size of the partition.
 * Thereafter two address values are used to indicate the address of the entry key and the entry
 * value.
 *
 * <p>At the entry key and entry value locations, an int is again used to announce the size (in
 * bytes) of either the key or value.
 *
 * <p>No attampts are made at making this map implementation thread-safe, but concurrent read
 * operations will not cause any concurrency issues.
 *
 * <p>The map accepts null values but not null keys.
 *
 * @author cfelde (Christian Felde, cfelde.com)
 */
@SuppressWarnings("restriction")
public class BOHMap implements Map<Binary, Binary> {
  public static final Logger log = LoggerFactory.getLogger(BOHMap.class);
  ...
  /**
   * Create a new binary off-heap hash map with the specified number of partitions. Assuming a
   * uniform distribution of key hashes each partition will contain approximately an equal share of
   * entries.
   *
   * <p>The number of partitions should be scaled with the number of expected map entries, in order
   * to maintain efficient lookup performance.
   *
   * <p>The default Arrays::hashCode method is used to calculate key hashes.
   *
  ...
  @Override
  public int size() {
    if (itemCount > Integer.MAX_VALUE) return Integer.MAX_VALUE;
  ...
  public Binary get(Object key) {
    ...
    for (long locationOffset = 0; locationOffset < entryCount; locationOffset++) {
      ...
      if (isEqual) {
        long valueAddress =
            unsafe.getAddress(locationAddress + (locationOffset * addressSize * 2) + addressSize);
  ...
  public Binary put(Binary key, Binary value) {
    ...
    for (long locationOffset = 0; locationOffset < entryCount; locationOffset++) {
      ...
      if (isEqual) {
        ...
        unsafe.putAddress(
            locationAddress + (locationOffset * addressSize * 2) + addressSize, valueAddress);
    ...
    if (locationAddress == 0) {
      ...
      locationAddress =
          unsafe.reallocateMemory(
              locationAddress, Integer.BYTES + (addressSize * 2 * (entryCount + 1)));
    }

    // Insert key and value pointers
    unsafe.putAddress(locationAddress + Integer.BYTES + (addressSize * 2 * entryCount), keyAddress);
    unsafe.putAddress(
        locationAddress + Integer.BYTES + (addressSize * 2 * entryCount) + addressSize,
        valueAddress);
    ...
    for (long locationOffset = 0; locationOffset < entryCount; locationOffset++) {
      ...
      if (isEqual) {
        ...
        if (locationOffset < entryCount - 1) {
          ...
          address =
              unsafe.getAddress(
                  locationAddress + ((entryCount - 1) * addressSize * 2) + addressSize);
          unsafe.putAddress(
              locationAddress + (locationOffset * addressSize * 2) + addressSize, address);
        }
  ...
  public void clear() {
    ...
    for (long offset = 0; offset < partitionCount; offset++) {
      ...
      for (long locationOffset = 0; locationOffset < entryCount; locationOffset++) {
        ...
        if (keyAddress != 0) unsafe.freeMemory(keyAddress);

        long valueAddress =
            unsafe.getAddress(locationAddress + (locationOffset * addressSize * 2) + addressSize);

        if (valueAddress != 0) unsafe.freeMemory(valueAddress);
      }
  ...
  protected void finalize() throws Throwable {
    ...
    //log.debug("Finalizing BOHMap with {} bytes. {} bytes left in unsafe storage", this.bytes, totalBytes);
    clear();
  ...
  private class KeySet implements Set<Binary> {
    ...
    public boolean removeAll(Collection<?> c) {
      ...
      for (Object k : c) {
        if (!(k instanceof Binary)) continue;
  ...
  private class KeySetIterator implements Iterator<Binary> {
    ...
    public Binary next() {
      ...
      long keyAddress =
          map.unsafe.getAddress(locationAddress + (locationOffset * map.addressSize * 2));
  ...
  private class ValuesIterator implements Iterator<Binary> {
    ...
    public Binary next() {
      ...
      long valueAddress =
          map.unsafe.getAddress(
              locationAddress + (locationOffset * map.addressSize * 2) + map.addressSize);
