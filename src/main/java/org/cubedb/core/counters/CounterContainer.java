...
public class CounterContainer {
	...
	public void initSideCounters() {
		...
		// group
		sideCounters = new ArrayBasedMultiDimensionalCounter(metricLookup.size(), fieldLookup.size(), fieldSizes);
	}
	...
	public void initGroupedSideCounters(String groupFieldName) {
		...
		// grouping
		sideCounters = new ListListMultiDimensionalCounter(metricLookup.size(), fieldLookup.size(), fieldSizes);
	}
	...
	public void saveResultTo(Map<GroupedSearchResultRow, Long> groupedResult) {
		...
		sideCounters.forEach((k0, k1, k2, k3, value) -> {
			...
			String groupFieldValue = isGroupLookup ? groupFieldLookup.getKey(groupFieldValueId)
					: SearchResult.FAKE_GROUP_FIELD_VALUE;

			GroupedSearchResultRow row = new GroupedSearchResultRow(groupFieldName, groupFieldValue, sideName,
					sideValue, metricName);
